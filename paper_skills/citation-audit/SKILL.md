---
name: citation-audit
description: "Zero-context verification that every bibliographic entry in the paper is real, correctly attributed, and used in a context the cited paper actually supports. Uses the AGENTS.md paper architect/reviewer role with web/DBLP/arXiv lookup to catch hallucinated authors, wrong years, fabricated venues, version mismatches, and wrong-context citations (cite present but the cited paper does not establish the claim). Use when user says \"审查引用\", \"check citations\", \"citation audit\", \"verify references\", \"引用核对\", or before submission to ensure bibliography integrity."
---
...
This skill is the bibliography layer of the evidence-and-claim assurance chain, complementing `experiment-result-to-claim` (science verdict) and `experiment-claim-audit` (numerical claims). Together they form an integrity stack from experimental evidence to manuscript bibliography.

## When to Use This Skill

**Run before submission.** The right gating point is:
- After `paper-write` has produced the LaTeX draft and bib file
- After `experiment-claim-audit` has verified numerical claims
...
- **REVIEW_ROLE = paper architect/reviewer** — Use the AGENTS.md paper review role with fresh context. Do not hard-code a model or backend.
- **CONTEXT_POLICY = `fresh`** — Each audit run uses a fresh reviewer context to reduce confirmation bias.
...
Save the extracted contexts to `paper/audit/citation-audit/contexts.txt` so the reviewer can read it directly. Use the paper-dir-relative path `audit/citation-audit/contexts.txt` when recording the file in `audited_input_hashes`; do not stage under `/tmp` or other transient locations that the local artifact gate cannot rehash later.

### Step 3: Send each entry to a fresh paper architect/reviewer

For each **cited** bib entry — i.e., each key in `cited_keys` with at least one extracted citation context — use a fresh paper architect/reviewer context. Do not reuse the same reviewer context across entries. Do **not** review entries in `bib_keys \ cited_keys`; those are detect-only and surface only when `--uncited` is explicitly enabled (see "Uncited Entry Detection" below).
...
Save the response to `paper/review-traces/citation-audit/<date>_runNN/<key>.md` per the review-tracing protocol.
...
`audited_input_hashes`, `trace_path`, `review_trace`, `review_role`,
...
- Downstream gates (`paper-writing` Phase 6) MUST NOT treat the presence of `uncited_entries` as a blocking signal.
...
- **Always emit, never block** — this skill always writes `CITATION_AUDIT.json` with a verdict; the decision to block finalization lives in `paper-writing` Phase 6, driven by the `assurance` level. See "Submission Artifact Emission" below.
...
|-------|---------------|-----------------|
| `/experiment-result-to-claim` | Result-to-claim mapping | Claims unsupported by evidence |
| `/experiment-claim-audit` | Numerical claims in manuscript | Number inflation, best-seed cherry-pick, config mismatch |
| `/citation-audit` | Bibliographic entries | Hallucinated refs, wrong-context citations, metadata errors |

Together: result → numerical claim → cited claim. Each layer uses fresh review context with no executor in the validator path.
...
After each reviewer call, save the trace following `../shared-references/review-tracing.md`. Write files directly to `paper/review-traces/citation-audit/<date>_run<NN>/`. Respect the `--- trace:` parameter (default: `full`).
...
- `paper/review-traces/citation-audit/<date>_runNN/` (per-entry review traces)
...
`paper-writing` Phase 6 relies on this artifact existing at a predictable path.
...
```json
{
  ...
  "trace_path":       "paper/review-traces/citation-audit/<date>_run<NN>/",
...
- `suggestion` is advisory only; `paper-writing` Phase 6 does not block on it.
...
`paper/audit/` so downstream gates can rehash reproducibly. Do NOT hash
...
downstream gate resolves relative to the paper dir; prefixing produces
...
Every invocation uses a fresh reviewer context. Never continue a prior review across
...
local artifact gate decide whether the verdict blocks finalization based on the
`assurance` level.

## See Also

- `/experiment-claim-audit` — sibling skill for numerical claim verification
- `/experiment-result-to-claim` — claim verdict assignment from results
- `../shared-references/citation-discipline.md` — protocol document for citation hygiene
- `../shared-references/reviewer-independence.md` — review independence constraints
