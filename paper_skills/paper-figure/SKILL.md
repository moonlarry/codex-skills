...
| **Architecture/pipeline diagrams** | ❌ No — use `paper-figure-archi` for a reference image, then redraw manually | Model architecture, data flow diagrams, system overviews. Use the generated reference as guidance for draw.io, Figma, PowerPoint, TikZ, or another editable tool |
...
**In practice:** For a typical ML paper, this skill handles ~60% of figures (all data plots + tables). The remaining ~40% (hero figure, architecture diagram, qualitative results) need manual or separate workflows. Use `paper-figure-archi` to create a raster reference for architecture diagrams, then redraw the final figure in an editable format and place it in `figures/` before running `/paper-write`. This skill will detect existing figures and preserve them.
...
- **REVIEW_ROLE = paper architect/reviewer** — Use the AGENTS.md paper review role for figure quality diagnosis. Do not hard-code a model.
...
Delegate figure descriptions and captions to the paper architect/reviewer role:
