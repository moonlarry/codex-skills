...
Systematically verify a mathematical proof via independent paper architect/reviewer audit, fix identified gaps, re-review until convergence, and generate a detailed audit report with proof-obligation accounting.
...
- MAX_REVIEW_ROUNDS = 3
- REVIEW_ROLE = paper architect/reviewer via the AGENTS.md paper workflow. Do not hard-code a model or backend.
...
### Phase 1: First Paper Architect/Reviewer Audit
...
### Phase 3: Paper Architect/Reviewer Re-Review

Use a fresh reviewer context for the next review round; proof-checker keeps each round independent. Request the same mandatory checklist.
...
For any fix that resolved a FATAL or CRITICAL issue, submit the **fixed section alone** (without showing the previous critique) to a **fresh paper architect/reviewer context**:
...
Phase 6 relies on this artifact existing at `<paper-dir>/PROOF_AUDIT.json`.
...
```json
{
  ...
  "trace_path":       "paper/review-traces/proof-checker/<date>_run<NN>/",
...
the reviewer's self-reported opened subset. The local artifact gate rehashes
these entries; any mismatch flags `STALE`.

**Path convention**: keys are
**paths relative to the paper directory** (no `paper/` prefix — the
local artifact gate resolves relative to the paper dir; prefixing produces
...
Every invocation uses a fresh reviewer context. Never continue a prior
proof-checker run. Do not accept prior audit outputs
