...
- **AUTO_EXPERIMENT = false** — When `true`, create a supplementary experiment plan when the strategy plan identifies reviewer concerns that require new empirical evidence. When `false` (default), pause and present the evidence gap to the user for manual handling.
...
2. Write the mini plan to `rebuttal/REBUTTAL_EXPERIMENT_PLAN.md` and ask the user before running or claiming any new results.

3. After the user provides completed results, update `ISSUE_BOARD.md`:
...
6. Use the prior rebuttal state and reviewer trace for continuity if useful
