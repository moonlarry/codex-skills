...
Use the two-role workflow required by the repository `AGENTS.md`:

- The paper architect/reviewer inspects experiment scope, result files, evaluation scripts when available, and claim compatibility.
- The paper executor collects paths, applies approved revisions, and writes the resulting plan, audit, or manuscript update.

Use `experiment-result-to-claim` to decide what a result can support. Use `experiment-claim-audit` to check whether manuscript claims, numbers, and tables overstate the evidence.

**Never let the executor be the only judge of its own experiment integrity.**
